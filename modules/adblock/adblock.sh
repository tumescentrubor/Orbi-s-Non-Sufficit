...
#
LC_ALL=C
PATH="/usr/sbin:/usr/bin:/sbin:/bin:/usr/local/adblock"
adb_ver="3.4.3"
adb_sysver="unknown"
adb_enabled=1
adb_debug=1
...
adb_triggerdelay=0
adb_backup=0
adb_backupdir="/usr/local/adblock/backup"
adb_fetchutil="wget"
adb_dns="dnsmasq"
adb_dnsprefix="adb_list"
adb_dnsfile="${adb_dnsprefix}.overall"
adb_dnsflush=0
adb_whitelist="/usr/local/adblock/adblock.whitelist"
...
f_envload()
{
	...
	dns_up="$( if ( nslookup localhost 127.0.0.1 | grep "Address 1" > /dev/null ); then echo true; fi )"
        if [ "${dns_up}" = "true" ]
        then
            break
        fi
